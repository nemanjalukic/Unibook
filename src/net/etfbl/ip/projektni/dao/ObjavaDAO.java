...
public class ObjavaDAO {
	...
	public static ArrayList<Objava> getAllObjave(User korisnik) {
		HashMap<Integer,Integer> liked = new HashMap<>();
		ArrayList<Objava> retVal = new ArrayList<>();
		liked=selectAllKorisnikLiked(korisnik.getId());
		retVal.addAll(selectAllDogadjaj());
		retVal.addAll(selectAllVijest());
		retVal.addAll(selectAllByKorisnik(korisnik));
		for(Objava obj : retVal) {
			Integer i=liked.get(obj.getId());
			if( i!=null) {
				obj.setOpcija(liked.get(obj.getId()));
			}
		}
		Collections.sort(retVal);
		comparator=new Comparator<Objava>() {

			@Override
			public int compare(Objava o1, Objava o2) {
				
				return new Integer((o2.getLikes()-o2.getDislikes())).compareTo( new Integer((o1.getLikes()-o1.getDislikes())));
			}
			
		};
		
		PriorityQueue<Objava> topStatus=new PriorityQueue<Objava>(5, comparator);
		topStatus.addAll(retVal);
		ArrayList<Objava> top5=new ArrayList<>();
		for(int i=0;i<5;i++) {
			top5.add(topStatus.poll());
		}
		
		retVal.removeAll(top5);
		top5.addAll(retVal);
